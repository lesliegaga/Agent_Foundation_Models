...
def default_compute_score(questions, ground_truths, responses, prompts, data_sources, extra_infos=None, use_llm_judge=False):
    if use_llm_judge:
        from . import llm_judge
        res = llm_judge.compute_score_llm_judge_batch(questions, responses, ground_truths, data_sources, prompts, extra_infos)
        return res
    else:
        from . import qa_em
        res = qa_em.compute_score_em_batch(data_sources, prompts, responses, ground_truths)
        return res
...
#     elif data_source in ["nq", "triviaqa", "popqa", "hotpotqa", "2wikimultihopqa", "2wiki", "musique", "bamboogle" \
#                          "webwalker", "gaia", "hle", "browsecomp"]:
#         from . import qa_em

#         res = qa_em.compute_score_em_batch(solution_str, ground_truth)
#     else:
