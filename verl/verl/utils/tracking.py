...
class Tracking:
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            wandb.init(project=project_name, name=experiment_name, config=config, dir=f"logs/wandb")
        ...
        if "tensorboard" in default_backend:
            self.logger["tensorboard"] = _TensorboardAdapter(name=experiment_name)
...
class _TensorboardAdapter:
    def __init__(self,name=None):
        ...
        tensorboard_dir = os.environ.get("TENSORBOARD_DIR", f"logs/tensorboard_log/{name}")
        os.makedirs(tensorboard_dir, exist_ok=True)
        print(f"Saving tensorboard log to path:{tensorboard_dir}")
...
class ValidationGenerationsLogger:
    ...
    def log_generations_to_swanlab(self, samples, step):
        ...
        for i, sample in enumerate(samples):
            ...
            if sample_len >= 5:  # input, output, score, data_source, ground_truth  
                row_text = f"""  
                input: {sample[0]}  
                ...
                """  
            elif sample_len >= 4:  # input, output, score, data_source  
                row_text = f"""  
                input: {sample[0]}  
