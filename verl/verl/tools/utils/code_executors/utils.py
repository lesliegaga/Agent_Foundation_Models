...
def parse_code_blobs_stdin_answer(code_blob: str) -> tuple[str,str]:
...
def parse_code_blobs(code_blob: str):
    """Parses the LLM's output to get any code blob inside. Will return the code directly if it's code."""
    pattern = r"```(?:py|python)?\n(.*?)\n```"
...
def make_init_file(folder: str):
    ...
    with open(os.path.join(folder, "__init__.py"), "w"):
        pass
def remove_from_solution_line(code_str):
    lines = code_str.split('\n')
    filtered_lines = [line for line in lines if not line.strip().lower().startswith('from solution')]
    return '\n'.join(filtered_lines)


def remove_metadata_and_keep_tests(code_str):
    lines = code_str.split('\n')
    filtered_lines = []
    skip = False
    ...
    for line in lines:
        ...
        if stripped or (line.lstrip().startswith('def ') if line.strip() else False):
            filtered_lines.append(line)
    
    return '\n'.join(filtered_lines)


def get_entry_point_from_solution(code_str):
    """
    For MBPP, extract the function name from the canonical_solution
    """
    import re
...
def extract_zero_arg_functions(code):
    lines = code.split('\n')
    function_names = []
    for line in lines:
        line = line.strip()
        if line.startswith('def') and 'test' in line:
            try:
                start_index = line.index('(')
                end_index = line.index(')')
                if start_index < end_index and line[start_index + 1:end_index].strip() == '':
...
def remove_main_block(code):
    ...
    cleaned_code = pattern.sub('', code)
    return cleaned_code
def try_extract_solution(solution_str):
    """
    Extract the content within the last <answer> </answer> tag from the solution
    """
    reversed_str = solution_str[::-1]
    reversed_pattern = r'>rewsna/<(.*?)>rewsna<'
    matches = list(re.finditer(reversed_pattern, reversed_str, re.DOTALL))
    if matches:
        reversed_content = matches[0].group(1).strip()
        final_answer = reversed_content[::-1]
        return final_answer
    return ""
def try_extract_solution_old(solution_str: str):
    ...
    return solution_str
def extract_last_code_from_string(solution_str):
    solution_str = try_extract_solution(solution_str)
    return parse_code_blobs(solution_str)