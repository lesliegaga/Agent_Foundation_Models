...
class AsyncRolloutRequest(BaseModel):
    ...
    tool_call_info: Dict[str, int] = {}  # Track tool calls per tool type
    detailed_tool_metrics: Dict = {}  # Detailed tool call tracking for metrics
    ...
    chat_template_style: str = "default"

    complete_reason: str
    finish_reason: dict
    web_search_history: List[str]
    crawl_page_history: List[str]
    ...
    def initialize_request(cls, values):
        ...
        tokens_without_prompt = tokenizer.apply_chat_template(messages,
                                add_generation_prompt=False, tokenize=True)
                                #  tools=tools,
        if not values.get("input_ids") or not values.get("attention_mask"):
            tokenization_dict_with_prompt = tokenizer.apply_chat_template(messages, 
        ...
        values["base_conv_wo_gen_prompt_end_pos"] = len(tokenizer.apply_chat_template(BASE_CHAT_HISTORY,  add_generation_prompt=False, tokenize=False))
        # tools=tools,
        values["base_conv_with_gen_prompt_end_pos"] = len(tokenizer.apply_chat_template(BASE_CHAT_HISTORY,add_generation_prompt=True, tokenize=False))
        #  tools=tools, 
        return values
    ...
    def get_generation_prompt_ids(self, tokenizer: PreTrainedTokenizer) -> list[int]:
        ...
        if self.use_inference_chat_template:
            return tokenizer.apply_chat_template([msg.model_dump() for msg in self.messages], 
    ...
    def add_tool_response_messages(self, tokenizer: PreTrainedTokenizer, contents: list[str]) -> None:
        ...
        content = tokenizer.apply_chat_template([*BASE_CHAT_HISTORY, *self.messages[-len(contents) :]], 
    ...
    def initialize_tool_metrics(self) -> None:
        """Initialize detailed tool metrics tracking."""
        self.detailed_tool_metrics = {
            "total_tool_calls": 0,
            "tool_calls_by_type": {},
            "conversation_turns": 0,
            "tool_call_sequence": [],  # Track the sequence of tool calls
            "unique_tools_used": set(),
        }
    
    def track_tool_call(self, tool_name: str, tool_call_id: str, arguments: Dict = None) -> None:
        """Track a tool call with detailed information."""
        if not hasattr(self, 'detailed_tool_metrics') or not self.detailed_tool_metrics:
            self.initialize_tool_metrics()
        
        # Update total count
        self.detailed_tool_metrics["total_tool_calls"] += 1
        
        # Update per-tool count
        if tool_name not in self.detailed_tool_metrics["tool_calls_by_type"]:
            self.detailed_tool_metrics["tool_calls_by_type"][tool_name] = 0
        self.detailed_tool_metrics["tool_calls_by_type"][tool_name] += 1
        
        # Track unique tools
        self.detailed_tool_metrics["unique_tools_used"].add(tool_name)
        
        # Track sequence
        self.detailed_tool_metrics["tool_call_sequence"].append({
            "tool_name": tool_name,
    ...
    def finalize_tool_metrics(self) -> Dict:
        """Finalize and return tool metrics for this trajectory."""
        if not hasattr(self, 'detailed_tool_metrics') or not self.detailed_tool_metrics:
            self.initialize_tool_metrics()
        
        # Count conversation turns (assistant messages)
        self.detailed_tool_metrics["conversation_turns"] = len([msg for msg in self.messages if msg.role == "assistant"])
        
        # Convert set to list for serialization
        self.detailed_tool_metrics["unique_tools_used"] = list(self.detailed_tool_metrics["unique_tools_used"])
        self.detailed_tool_metrics["unique_tools_count"] = len(self.detailed_tool_metrics["unique_tools_used"])
        
        return self.detailed_tool_metrics.copy()
    ...
    def finalize(
        ...
        if self.enable_tokenization_sanity_check:
            full_tokens = tokenizer.apply_chat_template([msg.model_dump() for msg in self.messages],
            add_generation_prompt=False, tokenize=True)
            # tools=([tool.model_dump() for tool in self.tool_schemas] if self.tool_schemas else None),
        ...
        print("[INFO] finalize one trajectory!")
